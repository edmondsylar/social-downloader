...
require('core/reason_phrases.php');
	...
	function PHP_LoadPage($page_url = '', $data = array(), $set_lang = true) {
		global $load, $lang_text, $lang, $config, $add_action, $CODE,$text_media, $text_media_two, $text_media ,$options_launcher;
...
//--
	function External_links($Link_info){
		...
		$Url_line = '';
		foreach (PHP_Get_url_line() as $key => $info) {
						...
						if ($plugin == null){
							...
							# Code	
								...
								$CODE['SHARE_EXTENSION'] = PHP_LoadPage('template/share_url', array(
										'DATA_URL' => $config['site_url'].'/s/'.PHP_data_ShareUrl_INSERT($url, $share_data, '0', $plugin),	
									));
								}
							# Code						
								...
								@$CODE['GET_URL_VALUE_LIST'] 		= $Data_media_Json['data_list'];
				...
				while (($file = readdir($dh)) !== false) {
					...
					if (substr($file,-1) != "." && pathinfo($file, PATHINFO_EXTENSION) == "php") {
						...
						if ($file_name == $lang)
							$line .= "";
							$line .= "";
							$line .= '<a href="'.$config['site_url'].'/lang/'.$url_lang.'"><li><span><img class="class_flag_icons" src="'.$img_lang.'" alt="'.$name_lang.'" title="'.$name_lang.'"></img> '.$name_lang.'</span></li></a>';
...
//--
	function PHP_add_comments($text, $email){
		global $con;
		$data_1 = PHP_Secure($text);
		$data_2 = PHP_Secure($email);
		$data_3 = time();
		$data_4 = PHP_Get_Public_ip();
		$data_5	= PHP_GetKey(25,25);
		$sqli	= "INSERT INTO ".T_COMMENTS." (comments,email,time,ip_user,key_comments)VALUES('$data_1','$data_2','$data_3','$data_4','$data_5');";
		mysqli_query($con, $sqli);
		if (!$sqli){
			$data_return = 204;
		}else{
...
//--
	function PHP_Update_YT_Key(){
		global $options_launcher;
		$fichero_sistem = './admin-panel/files/youtube/YT_lastjs.txt';
			
		if (file_exists($fichero_sistem)) {
			
			$time_update = 1750;
			
			$update_file = time() + $time_update;
			
			if($options_launcher['update_youtube_key'] < time()){
				@unlink($fichero_sistem);
				$launcher_data = array(
					'update_youtube_key'=> $update_file,
				);
