<!--div id="css_Download"-->
	...
	<?php foreach ($CODE['GET_URL_PLATFORM_MEDIA'] as $indice => $Data_media) { ?>
		<?php if($Data_media['size']=='ERROR' | $Data_media['size']=='e' | $Data_media['size']=='m' | $Data_media['size']=='O' | $Data_media['size']=='f' | $Data_media['size']==NULL){ ?>
			<!-- null -->
		<?php }else{ ?>
			<!-- list stories -->
			<?php if(@$CODE['GET_URL_VALUE_LIST'] == 'on'){ ?>
				<?php if($Data_media['quality'] == 'image'){ ?>
					<img class="img_list_css_resuta_html" src="<?php echo $Data_media['url']; ?>"></img>
				<?php }else if($Data_media['quality'] == 'video'){?>
					<!--video-->
				<?php } ?>
			<?php } ?>
			<!---->
			<div class="div_data_download_videos">
				...
				<?php if (@$CODE['GET_DIRECT_DOWNLOAD'] == NULL){ ?>
					<a class="button_video_more" href="{{CONFIG site_url}}/download.php?url_video=<?php echo PHP_DatesCrypt('encrypt', $Data_media['url']); ?>&title=<?php echo $CODE['GET_URL_VALUE_TITLE']; ?>&type_format=<?php echo $Data_media['format']; ?>&size_file=<?php echo $Data_media['size']; ?>" download="downloadfilename"><span id="btn_mo_tel">{{LANG Download_file}}</span></a>
				<?php }else{ ?>
