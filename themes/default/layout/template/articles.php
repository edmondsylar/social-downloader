			...
			<div class="box_page_home">
				...
				<div class="ccc_ma_0 content_video">
						...
						<div id="load_post_blogs">
							<?php
								if ($CODE['ADS'] != 'on') {
									echo $options_launcher['ads_three'];
								}
							?>
							<h2 class="text_blog text_align_bb_h2 h2_home">{{LOAD_TITLE}}</h2>
							<div class="post_redeee_size">
								{{LOAD_TEXT}}
								<?php if($CODE['NONE'] != 'on'){ ?>
									<p class="post_redeee">{{LANG article_text_3}}:</p>
								<?php } ?>
							</div>
							<?php if($CODE['NONE_LIST'] != 'on'){ ?>
								<h2 class="text_blog text_align_bb_h2 h2_home">{{LANG article_text_4}}</h2>
								<div id="results"></div>
							<?php } ?>							
							{{LOAD_POST_BLOGS}}
							<br>
