 ...
 if (isset($_GET['url'])) {
	...
	if($page == 'privacy'){
		...
		$load->content = PHP_LoadPage('template/pages', array(
			...
			'INFO_PAGES' => _Decode($load->config->privacy_policy),
		));
	}else if($page == 'copyright'){
		$load->title       = 'DMCA' .' | '. $load->config->title;
		$load->content = PHP_LoadPage('template/pages', array(
			'ICONS_MEDIA' => '',
			'DATA_PAGES' => 'DMCA',
			'INFO_PAGES' => _Decode($load->config->copyright),
		));	
		...
		$load->content = PHP_LoadPage('template/pages', array(
			...
			'INFO_PAGES' => _Decode($load->config->terms_of_use),
			));		
	} else if ($page == 'all-media') {
