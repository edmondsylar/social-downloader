...
$load->page        = 'watch';
 ...
 if (isset($_GET['url'])) {
	 	$id_url 			= PHP_Secure($_GET['v']);
		$data_url 			= 'https://www.youtube.com/watch?v='.$id_url.'';
	if($data_url == null){
		//-- 404
	}else{		
		...
		@$CODE['ACTIVE_WATCH'] = ($_GET['id']) ? 'off' : 'ok',
		$CODE['SHARE_EXTENSION'] = PHP_LoadPage('template/share_url', array(
										'DATA_URL' => $config['site_url'].'/s/'.$id_url,	
									)),
		@$CODE['watch_data'] = @$CODE['share_data'] = PHP_Url('youtube',$data_url,'return', false),
